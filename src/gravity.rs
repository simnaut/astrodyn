...
/// [`astrodyn_gravity::relativistic::compute_relativistic_correction`].
///
...
pub fn accumulate_relativistic_corrections<S: Copy + std::fmt::Debug + PartialEq>(
    ...
    for ctrl in &controls.controls {
        ...
        let other: Vec<astrodyn_gravity::relativistic::RelativisticSource> = controls
            .controls
            .iter()
            .filter(|c| c.source_name != ctrl.source_name)
            .filter_map(|c| {
                source_lookup(c.source_name).map(|s| {
                    astrodyn_gravity::relativistic::RelativisticSource {
                        mu: s.mu,
                        position: s.position,
                    }
                })
        ...
        total_correction += astrodyn_gravity::relativistic::compute_relativistic_correction(
            src.mu,
            src.position,
            body_position,
            body_velocity,
