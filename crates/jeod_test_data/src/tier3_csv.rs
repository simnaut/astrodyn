...
//! in `jeod_runner/tests/sim_test_helpers/mod.rs` into a single typed loader
...
/// Resolve a file under the workspace `test_data/` directory.
///
/// Walks up from `CARGO_MANIFEST_DIR` until it finds `Cargo.lock`, then joins
/// `test_data/<filename>`. Mirrors the lookup used by
/// [`crossval::output_dir`](super::crossval) so the resolution is consistent
/// regardless of whether tests run from a single-crate or workspace root.
pub fn test_data_path(filename: &str) -> PathBuf {
    ...
    loop {
        ...
        if !dir.pop() {
            // Fall back to the manifest dir's parent (legacy "../../test_data" path).
            return PathBuf::from(env!("CARGO_MANIFEST_DIR"))
                .join("../../test_data")
                .join(filename);
        }
    }
    dir.join("test_data").join(filename)
}
