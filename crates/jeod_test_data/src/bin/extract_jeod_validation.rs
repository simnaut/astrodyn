//! Extract / verify the `jeod_math` validation fixtures.
//!
...
//!   `jeod_test_data::leap_second::load_leap_second_table`).
...
//!   `jeod_test_data::mass_data::load_mass_data`).
//!
...
//! cargo run -p jeod_test_data --bin extract_jeod_validation -- --jeod-home /path/to/jeod
//! ```
//!
...
use jeod_test_data::euler_test::{encode_euler_cases_json, parse_euler_test_cases_cc};
use jeod_test_data::orbital_data::{encode_orbital_vectors_bin, parse_orbital_test_vectors_py};
...
fn extract_orbital_vectors(jeod_root: &Path, out_dir: &Path) {
    ...
    let meta = format!(
        "{{\n  \"source\": \"{rel}\",\n  \"vector_count\": {},\n  \"bytes\": {},\n  \
         ...
         \"note\": \"Regenerate with: cargo run -p jeod_test_data --bin extract_jeod_validation\"\n}}\n",
        vectors.len(),
        blob.len(),
    );
