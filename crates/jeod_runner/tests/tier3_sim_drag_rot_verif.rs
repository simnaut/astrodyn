...
use jeod_sim::{
    ...
    GravitySource, JeodQuat, MassProperties, MetAtmosphere, SimulationTime, TranslationalState,
};
...
fn tier3_simulation_drag_run6b_rotated() {
    ...
    let csv_path = test_data_path("dyncomp_run6b_rot_aero_traj.csv");
    assert!(
        csv_path.exists(),
    ...
    // Structural-to-body rotation: 15 deg about [1,1,1] (normalized).
    // Matches the eigen_angle override in generate_references.sh DYNCOMP_AERO_ROT_SNIPPET.
    ...
    let t_struct_body = q_struct_body.left_quat_to_transformation();
    ...
    println!(
        "Tier 3 (Simulation): SIM_dyncomp RUN_6B rotated structural frame, {} points",
        records.len()
    );
    ...
    report.assert_position([0.798, 1.114, 0.895]);
