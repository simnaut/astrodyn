...
//! `jeod_dynamics::frame_attach`. The Bevy adapter has the equivalent
//! glue in `bevy_jeod::frame_attach_system`.
//!
...
//! [`jeod_dynamics::derive_frame_attached_state`] — the same algebra
...
impl Simulation {
    ...
    pub fn attach_to_frame(
        ...
        self.bodies[body_idx].frame_attach = Some(FrameAttachState {
            parent_frame_id,
            attach_offset: jeod_dynamics::MassPointState {
    ...
    ///
    /// Drives [`jeod_dynamics::derive_frame_attached_state`] on each
