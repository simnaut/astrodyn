...
//! `astrodyn` Simulation runner) rotate to/from each body's structural
...
/// `Simulation::register_contact_pair` in `astrodyn_runner` get the same
...
/// (e.g. `astrodyn::evaluate_ground_contact_pair`) rotate them to
...
/// `astrodyn::evaluate_ground_contact_pair` or by
/// `Simulation::register_ground_contact_pair` in `astrodyn_runner`.
