...
//! `astrodyn_math::JeodQuat` still exposes convenience helpers for callers
...
///
/// Sealed at the type-system level: only `astrodyn_quantities` can impl
...
//
// These impls live here (rather than in `astrodyn_math::quaternion`) because
// inherent impls may only appear in the defining crate. See
// `astrodyn_math::quaternion` for the JEOD-specific `normalize_integ` variant
...
///
/// Single source of truth: `astrodyn_math::quaternion::NORM_LIMIT` re-exports
...
impl JeodQuat {
    ...
    /// version (see module docs of `astrodyn_math::quaternion`).
    ///
