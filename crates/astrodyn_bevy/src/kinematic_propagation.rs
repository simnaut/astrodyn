...
//!   ([`astrodyn::compute_kinematic_child_state`]) lives in
//!   `astrodyn_dynamics`.
//! - The orchestration walk
//!   ([`astrodyn::propagate_state_via_storage`]) lives in `astrodyn`.
    ...
    let mut nodes: HashMap<Entity, KinematicNodeState> = HashMap::with_capacity(view.len());
    {
        ...
        for entity in view.iter_entities() {
            ...
            let (rot_untyped, trans_untyped) = match read_q.get(entity) {
                Ok((r, t)) => (
                    crate::typed_bridge::rot_typed_to_raw(&r.0),
                    crate::typed_bridge::trans_typed_to_raw(&t.0),
                ),
    ...
    for (entity, state) in &derived {
        ...
        if kinematic_set.contains(entity) {
            if let Ok((mut rot_c, mut trans_c)) = writeback_q.get_mut(*entity) {
                ...
                rot_c.0 = crate::typed_bridge::rot_raw_to_self_ref(&state.rot);
                trans_c.0 = crate::typed_bridge::trans_raw_to_planet::<P>(&state.trans);
            }
        }
    }
}
...
/// `crates/astrodyn_runner/src/simulation/step/mod.rs`), which calls
    ...
    fn rotated_attach_child_tracks_parent_through_link() {
        ...
        let parent = app
            ...
            .spawn((
                ...
                RotationalStateC::from(crate::typed_bridge::rot_raw_to_self_ref(&(parent_rot))),
