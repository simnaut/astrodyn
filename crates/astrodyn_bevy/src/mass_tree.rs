...
//! [`astrodyn::MassStorage`] trait so the same composition kernel
//! (parallel-axis / Steiner) drives both the runner's arena
//! [`astrodyn::MassTree`] and the Bevy ECS world.
//!
//! Per the Three-Layer Architecture rule the *physics* lives in
//! `astrodyn_dynamics::mass_storage` and is re-exported via `astrodyn`;
...
//!
//! [#270]: https://github.com/simnaut/astrodyn/issues/270
//! [#271]: https://github.com/simnaut/astrodyn/issues/271
...
impl MassTreeQueries<'_, '_> {
    ...
    /// [`astrodyn::recompute_composites_via_storage`] without
    ...
    /// [`astrodyn::recompute_composites_via_storage`]. Returns the
...
/// [`astrodyn::MassStorage`]-driven kernel (see
/// `astrodyn_dynamics::mass_storage::recompute_composites_via_storage`),
...
/// `astrodyn::recompute_composites_via_storage` kernel, and writes
