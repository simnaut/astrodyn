...
pub const EARTH: PlanetShape =
    PlanetShape::new("Earth", EARTH_MU, EARTH_R_EQ, EARTH_R_POL, EARTH_FLAT_COEFF);

/// Moon. Constants from JEOD `moon.cc` and `moon_GRAIL150.cc`; numeric
/// values live in [`astrodyn_quantities::body_constants`].
pub const MOON: PlanetShape =
    PlanetShape::new("Moon", MOON_MU, MOON_R_EQ, MOON_R_POL, MOON_FLAT_COEFF);

/// Sun. Constants from JEOD `sun.cc` and `sun_spherical.cc`; numeric
/// values live in [`astrodyn_quantities::body_constants`].
pub const SUN: PlanetShape = PlanetShape::new("Sun", SUN_MU, SUN_R_EQ, SUN_R_POL, SUN_FLAT_COEFF);
