...
use astrodyn_test_data::{euler_test, orbital_data, orbital_init, reference_state};
...
fn validate_euler_matrix_from_jeod() {
    ...
    for (i, case) in cases.iter().enumerate() {
        ...
        let m = case.matrix;
        let mat = astrodyn_math::mat3_from_rows(
...
fn validate_euler_angle_extraction_from_jeod_vectors() {
    use astrodyn_math::{compute_euler_angles_from_matrix_typed, EulerSequence};
