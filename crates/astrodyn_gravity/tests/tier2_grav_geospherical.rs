...
//!
//! Validates `astrodyn_gravity` against the 40 static test cases shipped in
...
//! in `crates/astrodyn_runner/tests/tier3_*`.
//!
...
fn tier2_grav_geospherical_point_mass_sanity() {
    ...
    for case in &cases {
        let result = astrodyn_gravity::calc_spherical(mu_earth, case.position);
...
fn evaluate_case(data: &SphericalHarmonicsData, case: &GravityTestCase) -> (DVec3, f64, DMat3) {
    ...
    let sh_result = astrodyn_gravity::calc_nonspherical(
        data,
        case.position,
        degree,
        order,
    ...
    if case.perturb_only {
        ...
        let pm = astrodyn_gravity::calc_spherical(data.mu, case.position);
        (
...
fn tier2_grav_geospherical_surface_gravity_ggm02c() {
    ...
    let result_eq =
        astrodyn_gravity::calc_nonspherical(&data, pos_eq, data.degree, data.order, false, 0, 0);
    let pm_eq = astrodyn_gravity::calc_spherical(data.mu, pos_eq);
    ...
    let result_pol =
        astrodyn_gravity::calc_nonspherical(&data, pos_pol, data.degree, data.order, false, 0, 0);
    let pm_pol = astrodyn_gravity::calc_spherical(data.mu, pos_pol);
