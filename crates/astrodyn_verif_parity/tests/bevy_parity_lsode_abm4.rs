...
//!
//! The companion `RUN_lsode` case in `tier3_sim_lsode.rs` is **not**
//! mirrored here. Our integrator set does not yet include LSODE — the
//! `RUN_lsode` runner-side test compares JEOD's variable-order LSODE
//! reference against our fixed-order ABM4 with km-scale tolerances that
//! document the truncation difference between methods rather than
//! algorithm parity. There is no Bevy-side LSODE to assert parity
//! against until LSODE itself is ported (see
//! `crates/astrodyn_dynamics/src/abm4.rs` doc rationale for the
//! future-work scoping). The parity-coverage prefix rule treats this
//! file as coverage of the whole `lsode` tier3 topic — that is fine
//! at the meta-test layer (the `RUN_abm4` half is genuinely covered),
//! and the audit-trail note that LSODE-flavor parity is still
//! unbacked lives next to the dropped `DEFERRED_GAPS` entry in
//! `parity_coverage.rs` for future readers.
//!
//! ## Why not a recipe?
//!
