//! Tier 3: Bevy App vs astrodyn_runner::Simulation parity for the
//! planet-fixed frame's angular velocity.
//!
//! `astrodyn_runner::Simulation` writes `ang_vel_this = [0, 0, planet_omega]`
...
//!    `state.rot.ang_vel_this` in `astrodyn_runner::Simulation::frame_tree()`.
...
use astrodyn_bevy::{
    AstrodynPlugin, EphemerisR, FrameAngVelC, GravitySourceC, IntegrationDtR, PfixFrameEntityC,
...
/// Build a `astrodyn_runner::Simulation` with the same planet as `central_body`
...
/// Read the pfix node's `ang_vel_this` from `astrodyn_runner` for the
...
fn sim_pfix_ang_vel(sim: &Simulation, name: &str) -> DVec3 {
    ...
    let pfix_id = frame_tree
        ...
        .unwrap_or_else(|| panic!("no '{name}.pfix' frame node in astrodyn_runner frame tree"));
...
fn bevy_parity_planet_ang_vel_earth_rnp() {
    ...
    let bevy_ang_vel = app
        .world()
        .get::<PlanetAngularVelocityC<astrodyn::Earth>>(planet)
        .unwrap()
        .0
        .raw_si();
...
fn bevy_parity_planet_ang_vel_rotation_none_leaves_default() {
    ...
    let planet = app
        ...
        .spawn((
            ...
            PlanetAngularVelocityC::<astrodyn::Earth>::default(),
        ))
        .id();
    step_bevy_once(&mut app);
