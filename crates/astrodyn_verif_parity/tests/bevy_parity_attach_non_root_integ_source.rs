...
//! `crates/astrodyn_runner/src/simulation/mass_tree.rs` (`attach`'s
...
fn build_lunar_app() -> (App, Entity, Entity, Entity, astrodyn::MassBodyId) {
    let mut app = App::new();
    ...
    let parent_entity = app
        ...
        .spawn((
            ...
            TranslationalStateC::<astrodyn::Earth>::from(parent_initial_trans()),
    ...
    let child_entity = app
        ...
        .spawn((
            ...
            TranslationalStateC::<astrodyn::Earth>::from(child_initial_trans()),
            RotationalStateC::from(initial_rot()),
            FrameDerivativesC::default(),
            GravityControlsC(GravityControls { controls: vec![] }),
            IntegSourceC(Some(moon)),
...
// Per-body lift coverage where parent and child carry *distinct*
// integ origins (one in `PlanetInertial<Moon>`, one at root) belongs
// to the cross-integ-frame branch of `staging_system`. On this
// branch that path is gated by a fail-loud fence — its panic is
// pinned by the cross-integ-frame guard in
// `bevy_parity_attach_detach_momentum.rs`. A "same-source"
// variant cannot exercise distinct origins by construction (both
// bodies share the same `IntegSourceC` and therefore the same
// `IntegOrigin`); the shared-origin per-body-lift surface is
// already pinned end-to-end by
// `bevy_parity_attach_non_root_integ_source_lift_and_lower` above.
