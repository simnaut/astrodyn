...
//! is bit-identical to `astrodyn_runner::Simulation` configured with the
...
use astrodyn::{
    ...
    GravityGradient, GravityModel, GravitySource, GravitySourceEntry, InertiaTensor, JeodQuat,
    MassPropertiesTyped, Position, RotationalStateTyped, SelfRef, SixDofState, SrpModel,
    StructuralFrame, TranslationalState, Vec3Ext, VehicleConfig, EARTH, MOON,
};
use astrodyn_bevy::{
...
fn bevy_parity_integ_source_lunar_orbit_matches_simulation() {
    ...
    // without this call.
    register_planet_systems::<astrodyn::Moon>(&mut app);
    ...
    let vehicle = app
        ...
        .spawn((
            ...
            GravityControlsC(GravityControls {
                controls: vec![
                    {
                        let mut c = GravityControl::new_spherical(_earth, GravityGradient::Skip);
                        c.differential = true;
                        c
                    },
                    GravityControl::new_spherical(moon, GravityGradient::Skip),
                ],
            }),
    ...
    // `astrodyn_runner::Simulation::set_source_position`). The mutator's
    ...
    sim.add_body(VehicleConfig {
        ...
        gravity_controls: GravityControls {
            controls: vec![
                {
                    let mut c = GravityControl::new_spherical(0_usize, GravityGradient::Skip);
                    c.differential = true;
                    c
                },
                GravityControl::new_spherical(moon_idx, GravityGradient::Skip),
            ],
        },
...
/// `astrodyn_runner::Simulation`'s post-integration `body.solar_beta` for
...
/// `astrodyn_runner::Simulation`'s post-step state for the same
