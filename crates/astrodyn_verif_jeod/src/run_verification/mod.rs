...
//! ([`sim_dyncomp`], etc.). Each test in `astrodyn_runner/tests/tier3_*.rs`
...
/// without depending on the `astrodyn_runner` crate directly.
    ...
    fn observe(
        ...
        match (&self.kind, records) {
            ...
            (ExtrasComparator::Euler, CsvRecords::Euler(recs)) => {
                let r = &recs[idx];
                let jeod_q = astrodyn::JeodQuat::new(
                    r.quaternion[0],
                    r.quaternion[1],
                    r.quaternion[2],
                    r.quaternion[3],
            ...
            (ExtrasComparator::DyncompEuler, CsvRecords::Dyncomp(recs)) => {
                ...
                let jeod_q = astrodyn::JeodQuat::new(q.w, q.x, q.y, q.z);
                self.observe_euler_from_quat(body, &jeod_q, idx, case_name);
            }
