...
fn load_relative_csv(path: &std::path::Path) -> Vec<RelativeRecord> {
    ...
    for (i, line) in content.lines().enumerate() {
        ...
        // Columns (57 total, interleaved):
        // 0: time
        // 1-6: vehA pos/vel interleaved (pos[0],vel[0],pos[1],vel[1],pos[2],vel[2])
        // 7-10: vehA quaternion (scalar, vector[0-2]) — first of 4 duplicates
        // 24-26: vehA ang_vel[0-2]
        // 27-32: vehB pos/vel interleaved
        // 33-36: vehB quaternion — first of 4 duplicates
        // 49-51: vehB ang_vel[0-2]
        // 52-54: rel position[0-2]
        // 55-57: rel velocity[0-2]
        records.push(RelativeRecord {
            ...
            veh_b_pos: DVec3::new(p(27), p(29), p(31)),
            veh_b_vel: DVec3::new(p(28), p(30), p(32)),
            veh_b_quat: JeodQuat::new(p(33), p(34), p(35), p(36)),
            jeod_rel_pos: DVec3::new(p(52), p(53), p(54)),
            jeod_rel_vel: DVec3::new(p(55), p(56), p(57 - 1)), // col 56 is last vel
        });
    }
    records
}
...
fn run_relative_scenario(label: &str, csv_name: &str) {
    ...
    // JEOD's relative state "A w.r.t. B in B" is expressed in B's body frame,
    // while our compute_relative_state uses inertial frame subtraction.
    // Loose tolerance validates pipeline connectivity and order-of-magnitude;
    // tighten after rotating relative state into B's frame.
    assert!(
        max_pos_err < 1000.0,
        "{label}: max position error {max_pos_err:.4e} m exceeds 1000 m"
    );
    assert!(
        max_vel_err < 200.0,
        "{label}: max velocity error {max_vel_err:.4e} m/s exceeds 200 m/s"
    );
}
