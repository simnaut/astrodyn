...
- [ ] **Tier 3 torque_simple retightened**: With 3rd-body differential acceleration ported, re-enable Sun/Moon gravity (mu > 0) in `tier3_sim_torque_simple.rs` and tighten thresholds: position < 0.5 m, quaternion < 0.01 rad, torque < 1e-2 N·m across all 6 SIM_torque_compare_simple runs. The Phase 4a thresholds (100 m / 0.1–1.0 rad / 10–200 N·m) were inflated by missing 3rd-body perturbation cascading through gravity gradient feedback.
