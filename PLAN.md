...
- [x] **Euler edge cases**: Euler angles match JEOD to < 0.02 rad in eccentric and equatorial orbits over 24h (SIM_Euler RUN_ecc/RUN_equ). Quaternion error < 0.01 rad; Euler error derives from quaternion tracking (same regime as Phase 3a RUN_inc).
...
- [x] **Drag model variants**: All three Cd modes (constant, variable, ballistic coefficient) produce non-zero drag forces with plausible magnitudes for ρ=1e-12 kg/m³ (SIM_VER_DRAG). Full force comparison deferred pending implementation of JEOD's exact plate-model drag API in our code.
