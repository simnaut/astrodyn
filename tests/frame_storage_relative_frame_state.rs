...
//! mission-code ergonomic improvement of the new surface alongside
...
/// Validate that the dual-write keeps `FrameTransC` exactly in sync
/// with the arena's `state.trans` for the body frame.
#[test]
...
/// Validate that the dual-write keeps the pfix frame entity's
...
/// `FrameTreeR.compute_relative_state(planet_inertial_id, body_id).trans.position`.
///
/// This is the **load-bearing** validation: it proves the new
/// SystemParam read path produces bit-identical numerics to the arena
/// read path. The design doc cites this test as the "After" surface's
...
/// body. Computes (pfix → body), traversing pfix up to planet inertial
