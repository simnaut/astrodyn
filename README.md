...
`cargo test --workspace` works on a fresh clone without `$JEOD_HOME` —
all reference data is committed under `test_data/`. `JEOD_HOME` is only
required when regenerating fixtures via the `extract_*` binaries under
