...
| DB.14 | Integration frame switch delegates to root body | structural | consistency | partial (`bevy_jeod::staging_system` writes the post-attach composite-body inertial state to the parent's `TranslationalStateC` / `RotationalStateC`; the parent now integrates the merged body. Frame-side switching still belongs to #280) |
