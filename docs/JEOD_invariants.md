...
| DB.13 | State propagation delegates to root body | structural | consistency | partial (`bevy_jeod::staging_system` runs `combine_states_at_attach` on `AttachEvent` and writes the merged composite-body state to the parent — the integrated body now owns the whole subtree's translational + rotational state; `jeod_runner::Simulation::attach_subtree_aligned` is the runner equivalent. Frame-tree side and child-frame derivation deferred to Phase 5 / #198) |
...
| DB.17 | Only root body computes total acceleration | structural | structural | partial (`src/wrench.rs` aggregates child wrenches into the root, zeroes non-root `TotalForceC` / `FrameDerivativesC`, AND inserts `KinematicChildC` on every non-root chain member; `src/components.rs::KinematicChildC` and `src/systems.rs::integration_system`'s `Without<KinematicChildC>` filter together gate non-root nodes out of integration so they cannot drift under gravity. The kinematic-only propagation that derives child poses from the root each step is the sister `propagate_state_from_root_system` and not yet implemented — until it lands, kinematic children's `TranslationalStateC` / `RotationalStateC` stays frozen at the value the system observed when the body became kinematic.) |
