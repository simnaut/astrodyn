...
| IN.35 | GroundFacet `active` flag gates contribution at runtime; inactive ground facets produce zero force (JEOD `verif/SIM_ground_contact/models/contact_ground/src/contact_ground.cc:88` skips inactive `GroundInteraction`s inside `check_contact_ground`) | fatal | runtime | enforced (`crates/jeod_interactions/src/contact.rs::compute_ground_contact_geometry` asserts `ground_facet.active`; `Simulation::register_ground_contact_pair` also asserts at registration time) |
