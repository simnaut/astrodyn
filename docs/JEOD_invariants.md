...
| RF.11 | Gravitational parameter μ must match the planet whose inertial-frame position/velocity it is paired with at typed orbital-mechanics consumers (`OrbitalElements::from_cartesian_typed`, gravity sources, relativistic corrections, solar beta). A μ for the wrong central body silently produces an orbit / frame mismatch at the numeric level. | structural (typed surfaces) + convention (registry-side) | consistency | partial — structural at the typed surface (`GravParam<P>` carries the planet phantom; `OrbitalElements::from_cartesian_typed` requires `GravParam<P>` and `Position/Velocity<PlanetInertial<P>>` to share `<P>`, returning `OrbitalElements<P>` so the planet identity flows through to downstream consumers; the `mu_*()` constants in `jeod_sim::recipes::constants` return planet-pinned `GravParam<Earth>` / `<Sun>` / `<Moon>` / `<Mars>`). The runner's gravity registry, `PlanetShape::mu_typed()`, and `PlanetConfig::mu_typed()` carry a `SelfPlanet`-tagged μ — the planet is determined at runtime by which entity references the source. The `relabel::<P>` escape hatch is the explicit boundary between the dynamic-registry and static-typed worlds. See issue #303. |
