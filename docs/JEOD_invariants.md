...
| DB.17 | Only root body computes total acceleration | structural | structural | partial (`src/wrench.rs` zeroes non-root `TotalForceC` / `FrameDerivativesC` after aggregation so only the root's total drives integration; the kinematic-only propagation that derives child poses from the root each step is the sister `propagate_state_from_root_system` and not yet implemented) |
