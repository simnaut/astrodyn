...
| IN.38 | Contact-pair registration is initialization-only. JEOD does not enforce this inside `Contact::register_contact` or `ContactGround::register_ground_facet` — those methods are public and contain no init-state guard — so the guarantee is *purely conventional* via Trick's S_define job phasing: `P_BODY("initialization")` / `P_DYN("initialization")` (`SIM_ground_contact/S_modules/sv_dyn.sm:130-133`, `contact.sm:70-72`) runs the registration call exactly once before the integration loop. Calling registration mid-run on our side would inject the t=0-state init-phase impulse stored in `pending_initial_impulse` into a running trajectory; we therefore enforce the invariant inside `Contact` itself, which is *stricter than JEOD* (in keeping with the *Fail Loudly* policy in CLAUDE.md). | fatal | initialization | enforced (`Simulation::register_contact_pair` and `Simulation::register_ground_contact_pair` assert `!self.has_stepped`; flag flipped at the top of `step_internal`) |
