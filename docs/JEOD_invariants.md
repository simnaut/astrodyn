...
| DB.21 | Only unattached bodies integrate | flag-gate | runtime | enforced (`bevy_jeod::DetachedSubtreeStateC` + `bevy_jeod::step_detached_system` advance detached subtrees ballistically while integrators run on the integrated body's composite state; `jeod_runner::Simulation::detached_subtrees` is the runner equivalent. **Frame-attached bodies (issues #198 / #206):** `crates/jeod_runner/src/simulation/frame_attach.rs::Simulation::attach_to_frame` populates `SimBody.frame_attach`, and the runner's `step::integrate.rs` skips the integrator on `if body.frame_attach.is_some()` — mirrors JEOD `dyn_body_integration.cc:309-333`'s `frame_attach.isAttached()` branch. Bevy mirror: `src/components/frame_attach.rs::FrameAttachedC` marker plus the `Without<FrameAttachedC>` filter on `integration_system` (`src/systems/integration.rs`), with `src/frame_attach_system.rs` driving the per-tick state derivation. Mass-tree-attached children are already gated by `KinematicChildC`; `DB.21` is the disjunction of those three cases. Mutual exclusion between mass-tree attach and frame-tree attach is enforced at the API boundaries (`Simulation::attach_to_frame` / `frame_attach_system` panic on overlap).) |
...
| DB.24 | Default `integrated_frame` is composite_body | structural | structural | structural (`src/components/state.rs`, we integrate composite_body state) |
...
| DB.28 | Forces collected in structural frame, rotated to inertial at root | structural | consistency | enforced (`src/systems/force_collection.rs` force_collection_system: T_inertial_struct^T * struct_force) |
| DB.29 | Torques collected in structural frame, rotated to body at root | structural | consistency | enforced (`src/systems/force_collection.rs` force_collection_system: T_struct_body * struct_torque) |
...
| DB.31 | Frame-attached bodies derive composite-body state by composing the captured parent → struct offset with the body's struct → composite CoM offset (`models/dynamics/dyn_body/src/dyn_body_propagate_state.cc:571`'s `compute_derived_state_forward(structure, mass.composite_properties, composite_body)` call inside `propagate_state_from_structure`, called after `dyn_body_integration.cc:325-332` rebuilds `X_iframe_to_struct = X_iframe_to_pframe ⊕ attach_offset`) | structural | consistency | enforced (`crates/jeod_dynamics/src/frame_attach.rs::derive_frame_attached_state` chains `propagate_forward(parent, attach_offset)` then `propagate_forward(struct_state, composite_offset)`; the runner's `crates/jeod_runner/src/simulation/frame_attach.rs::Simulation::propagate_frame_attached_state` and the Bevy adapter's `src/frame_attach_system.rs::propagate_frame_attached_state_system` both populate `composite_offset` from the body's `MassProperties::position` / `t_parent_this` (zero / identity for atomic point-mass bodies, the SIM_dyncomp ISS fixture's `(-3, -1.5, 4)` for the 5.22 m surface-attach offset). Without this step the composite-body writeback would land the CoM-offset magnitude away from JEOD's value whenever the body's CoM is offset from the structural origin — exactly the surface-attach residual the SIM_dyncomp `RUN_attach_to_ref_frame` `attached_surface_pt` / `attached_surface_matrix` windows exhibited before this enforcement landed.) |

## Section MA: Mass / MassBody / MassProperties

| Tag | Invariant | Enforcement | Category | Our Status |
|-----|-----------|-------------|----------|------------|
| MA.01 | MassBody always present on DynBody (value member) | structural | structural | enforced (`src/validation.rs`, `src/systems/glue/frame_registration.rs`) |
| MA.02 | mass > 0 for meaningful dynamics | conditional | consistency | enforced (`crates/jeod_dynamics/src/mass.rs`, `src/systems/glue/frame_registration.rs`) |
...
| MA.16 | 180° yaw convention for attach-by-point | structural | structural | enforced (`MassTree::attach_aligned` in `crates/jeod_dynamics/src/mass_body.rs` hardcodes the `t_yaw = diag(-1, -1, 1)` middle leg of the docking-chain composition for the body→body case, mirroring JEOD `mass_attach.cc:112-115`. The body→reference-frame branch — `BodyAttachAligned`'s `ref_parent` dispatch (`models/dynamics/body_action/src/body_attach_aligned.cc:111-126`) — is enforced by `MassTree::attach_to_frame_offset_aligned` (`crates/jeod_dynamics/src/mass_body.rs`) and the runner-side wrapper `Simulation::attach_to_frame_aligned` (`crates/jeod_runner/src/simulation/frame_attach.rs`). Both branches honour JEOD's hardcoded `(zero offset, diag(-1,-1,1))` parent-point pose at the cpt and reduce the named-point algebra back to the same `(offset_pframe_struct, T_pframe_struct)` that the matrix form would supply directly.) |
...
| GV.12 | Gravity source must exist for control | error | initialization | enforced (`src/validation.rs` + `src/systems/environment.rs` panic) |
| GV.13 | Gravity source must have inertial frame | error | initialization | enforced (`src/systems/environment.rs` panics if nonspherical without PlanetFixedRotationC) |
...
| AT.03 | Planet-fixed position required for geodetic altitude | structural | runtime | enforced (`src/systems/environment.rs` — panics if planet_entity set but PlanetFixedRotationC missing) |
| AT.04 | Wind velocity computed as omega × position (co-rotation) | structural | runtime | enforced (`crates/jeod_atmosphere/src/lib.rs` compute_corotation_wind, `src/systems/environment.rs` atmosphere_update_system) |
...
| IN.09 | RadiationSource planet must be found by DynManager | fatal | initialization | enforced (`src/systems/interaction.rs` — panics on multiple SunMarker; zero SunMarker = SRP not configured, early return like JEOD `active=false`) |
...
| IN.15 | Aero drag requires body orientation (T_inertial_struct) | structural (mandatory fn parameter) | runtime | enforced (`src/systems/interaction.rs` — panics if AerodynamicForceC present without RotationalStateC) |
...
| Tag | Invariant | Enforcement | Category | Our Status |
|-----|-----------|-------------|----------|------------|
| FD.01 | `trans_accel = non_grav_accel + grav_accel` | structural | consistency | enforced (`src/systems/force_collection.rs` force_collection_system writes FrameDerivativesC) |
| FD.02 | `rot_accel = I^-1 * (tau - omega x I*omega)` | structural | consistency | enforced (`src/systems/force_collection.rs` force_collection_system writes FrameDerivativesC) |
...
| BA.09 | `BodyAction::is_ready()` is consulted on every `perform_actions` pass; not-ready actions stay queued (`perform_actions.cc:45-62`) | structural | ordering | enforced (`bevy_jeod::body_action_system` calls `BodyAction::is_ready` each tick and only mutates the subject when true; queue retains pending entries — see `crates/jeod_sim/src/body_action.rs::is_ready` and `src/body_action.rs::body_action_system`) |
| BA.10 | `DynManager::remove_body_action` removes the **first** pending action by `action_name`; empty name no-ops (`dyn_manager.cc:205-221`) | structural | initialization | enforced (`bevy_jeod::body_action_intake_system` skips an empty `name` (`Remove { name: "" }` is a no-op, matching JEOD `dyn_manager.cc:207-209`), then retains-by-name and drops *every* matching pending entry — a strict generalisation of the linear-scan-and-erase loop in JEOD. Covered by `bevy_jeod::body_action::tests::remove_drops_all_pending_with_matching_name` and `tests::empty_name_remove_is_noop`) |
| BA.11 | `DynBodyInitLvlhRotState`/`DynBodyInitLvlhState` compose inertial→LVLH (from a reference orbit) with LVLH→body (user input) and the LVLH-relative angular velocity to produce inertial→body attitude and body-frame angular velocity. Translational user inputs are stripped before apply (`dyn_body_init_lvlh_rot_state.cc:65-105`, `dyn_body_init_lvlh_state.cc:88-189`, `dyn_body_init.cc:273-328`, `RefFrameState::incr_left` lines 235-302) | structural | initialization | enforced (`jeod_dynamics::body_init::init_rot_from_lvlh` reproduces the `Q_A:C = Q_B:C * Q_A:B` quaternion compose and the `w_A:C = T_B:C * w_A:B + w_B:C` ang-vel compose; `BodyAction::InitLvlhRot` carries only the rotational sub-spec so the JEOD `RefFrameItems::Pos_Vel` strip is structural — there is no translational field on the variant. Covered by `crates/jeod_dynamics/src/body_init.rs::tests::lvlh_rot_*` and `tests/bevy_parity_body_action_init_lvlh_rot.rs`) |
| BA.12 | Chained attach re-roots the subject's existing tree under the new parent (`dyn_body_attach.cc:521-567` — when `child_root != &child`, the subject's tree-root replaces the subject itself in the topology, and the offset/rotation are recomputed so the subject's structural frame ends up at the user-requested pose under the new parent) | structural | consistency | enforced (`MassTree::attach_with_reroot` in `crates/jeod_dynamics/src/mass_body.rs`; runner-side `Simulation::attach_inner` dispatches through it and uses the subject root's pre-attach `body.trans` / `body.rot` as the child side of the momentum-conservation combine — `crates/jeod_runner/src/simulation/mass_tree.rs`; Bevy-side `staging_system` (`src/systems/integration.rs`) dispatches every `AttachEvent` through the same kernel and reparents the rerooted subtree's `MassChildOf` Relations under the new parent so the entity-side composition path observes the same shape as the arena tree. The auto-flag of the subject subtree as `kinematic_only` post-attach (runner) / `MassChildOf`-driven `KinematicChildC` insertion via `wrench_aggregation_system` (Bevy) maintains JEOD_INV: DB.17 — only the merged tree's root integrates) |
...
(`crates/jeod_quantities`) that have no JEOD C++ analogue. JEOD uses naked
...
| Tag | Invariant | Enforcement | Category | Our Status |
|-----|-----------|-------------|----------|------------|
| TS.01 | `<SelfRef>` and `<SelfPlanet>` wildcards appear **only at per-entity storage boundaries** (Bevy `Component`s, `Message`s, runner `SimBody`/`VehicleConfig`/`VehicleOutput` fields) and at the boundary types those storage forms wrap (e.g. `FlatPlateState<V>` inside a `FlatPlateConfigC`, `OrbitalElements<P>` inside a runner output). The wildcard exists because the entity / runner-state slot decides the vehicle/planet identity at runtime — the type system cannot statically know which mission's vehicle is sitting in any given `Entity`. **All system code paths and APIs use concrete or generic-bounded phantoms**: `<P: Planet>` and `<V: Vehicle>` parameters that flow from the call site, never `<SelfRef>` / `<SelfPlanet>` minted afresh in a system body. The auto-memory rule "No default type parameters on Planet-aware types" (`<P: Planet = Earth>` and friends) is the inverse-direction sister rule; both rules together close the loop: defaults silently relax to `<SelfPlanet>` when inference has no constraint, hiding a missing pinning decision; explicit wildcards at runtime-resolved boundaries are not. A drive-by `let foo: RelativeState<SelfRef, SelfRef> = …` inside a non-storage code path silently degrades type safety with no compiler signal; this invariant is what catches that. **Storage-boundary sites** that legitimately mint the wildcard: the `<SelfRef>`-tagged Bevy `Component` field types under `src/components/` — `RotationalStateC`, `MassPropertiesC`, `TotalForceC`, `FrameDerivativesC`, `StructuralTransformC`, `ExternalTorqueC` in `src/components/state.rs`; `GravityTorqueC` in `src/components/gravity.rs`; `FlatPlateConfigC` in `src/components/interaction.rs`; the `Message` types `AttachEvent<VParent, VChild>` and the canonical `<SelfRef, SelfRef>` Bevy adapter registration in `src/lib.rs`; the runner `SimBody` / `VehicleConfig` / `VehicleOutput` fields in `crates/jeod_runner/src/simulation/types.rs` (`flat_plate_state: Option<FlatPlateState<SelfRef>>`, `atmospheric_state: Option<AtmosphereState<SelfPlanet>>`, `orbital_elements: Option<OrbitalElements<SelfPlanet>>`); the `<SelfPlanet>`-erased runner-side `compute_orbital_elements` / `evaluate_atmosphere` returns in `crates/jeod_sim/src/{derived,atmosphere}.rs` and `crates/jeod_sim/src/planet_config.rs::PlanetConfig::mu_typed`. **Non-storage uses are forbidden** anywhere outside the allow-list — see `tests/self_ref_self_planet_discipline.rs` for the lint that enforces this and the precise file/comment marker mechanism (`// JEOD_INV: TS.01` either inline at the use, on a preceding pure-comment line, or as a file-level `//! JEOD_INV: TS.01 …` module-doc opt-in for files whose entire purpose is the storage boundary). | structural (no JEOD analogue) + lint | structural | enforced (`crates/jeod_quantities/src/frame.rs` defines `SelfRef` / `SelfPlanet` with the runtime-resolved-boundary docstring; the per-stage submodules `src/components/state.rs`, `src/components/gravity.rs`, `src/components/interaction.rs`, `src/components/mass_tree.rs` plus their re-exporting `src/components/mod.rs` carry the Bevy `Component` boundary tags; `src/lib.rs` registers the canonical `AttachEvent<SelfRef, SelfRef>` Bevy message; `crates/jeod_runner/src/simulation/types.rs` carries the runner storage tags; `crates/jeod_sim/src/derived.rs`, `crates/jeod_sim/src/atmosphere.rs`, and `crates/jeod_sim/src/planet_config.rs` carry the dynamic-registry-erased return-type tags; `tests/self_ref_self_planet_discipline.rs` is the bidirectional CI lint that refuses any new `SelfRef` / `SelfPlanet` token outside the allow-list mechanism described above) |
