...
| BA.10 | `DynManager::remove_body_action` removes the **first** pending action by `action_name`; empty name no-ops (`dyn_manager.cc:205-221`) | structural | initialization | enforced (`bevy_jeod::body_action_intake_system` retains-by-name; we drop *every* matching pending entry, a strict generalisation of the linear-scan-and-erase loop in JEOD — covered by `bevy_jeod::body_action::tests::remove_drops_all_pending_with_matching_name`) |
