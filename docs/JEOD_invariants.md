...
| Tag | Invariant | Enforcement | Category | Our Status |
|-----|-----------|-------------|----------|------------|
| TS.01 | `<SelfRef>` and `<SelfPlanet>` wildcards appear **only at per-entity storage boundaries** (Bevy `Component`s, `Message`s, runner `SimBody`/`VehicleConfig`/`VehicleOutput` fields) and at the boundary types those storage forms wrap (e.g. `FlatPlateState<V>` inside a `FlatPlateConfigC`, `OrbitalElements<P>` inside a runner output). The wildcard exists because the entity / runner-state slot decides the vehicle/planet identity at runtime — the type system cannot statically know which mission's vehicle is sitting in any given `Entity`. **All system code paths and APIs use concrete or generic-bounded phantoms**: `<P: Planet>` and `<V: Vehicle>` parameters that flow from the call site, never `<SelfRef>` / `<SelfPlanet>` minted afresh in a system body. The auto-memory rule "No default type parameters on Planet-aware types" (`<P: Planet = Earth>` and friends) is the inverse-direction sister rule; both rules together close the loop: defaults silently relax to `<SelfPlanet>` when inference has no constraint, hiding a missing pinning decision; explicit wildcards at runtime-resolved boundaries are not. A drive-by `let foo: RelativeState<SelfRef, SelfRef> = …` inside a non-storage code path silently degrades type safety with no compiler signal; this invariant is what catches that. **Storage-boundary sites** that legitimately mint the wildcard: the `<SelfRef>`-tagged Bevy `Component` field types under `crates/astrodyn_bevy/src/components/` — `RotationalStateC`, `MassPropertiesC`, `TotalForceC`, `FrameDerivativesC`, `StructuralTransformC`, `ExternalTorqueC` in `crates/astrodyn_bevy/src/components/state.rs`; `GravityTorqueC` in `crates/astrodyn_bevy/src/components/gravity.rs`; `FlatPlateConfigC` in `crates/astrodyn_bevy/src/components/interaction.rs`; the `Message` types `AttachEvent<VParent, VChild>` and the canonical `<SelfRef, SelfRef>` Bevy adapter registration in `crates/astrodyn_bevy/src/lib.rs`; the runner `SimBody` / `VehicleConfig` / `VehicleOutput` fields in `crates/astrodyn_runner/src/simulation/types.rs` (`flat_plate_state: Option<FlatPlateState<SelfRef>>`, `atmospheric_state: Option<AtmosphereState<SelfPlanet>>`, `orbital_elements: Option<OrbitalElements<SelfPlanet>>`); the `<SelfPlanet>`-erased runner-side `compute_orbital_elements` / `evaluate_atmosphere` returns in `src/{derived,atmosphere}.rs` and `src/planet_config.rs::PlanetConfig::mu_typed`. **Non-storage uses are forbidden** anywhere outside the allow-list — see `tests/self_ref_self_planet_discipline.rs` for the lint that enforces this and the precise file/comment marker mechanism (`// JEOD_INV: TS.01` either inline at the use, on a preceding pure-comment line, or as a file-level `//! JEOD_INV: TS.01 …` module-doc opt-in for files whose entire purpose is the storage boundary). | structural (no JEOD analogue) + lint | structural | enforced (`crates/astrodyn_quantities/src/frame.rs` defines `SelfRef` / `SelfPlanet` with the runtime-resolved-boundary docstring; the per-stage submodules `crates/astrodyn_bevy/src/components/state.rs`, `crates/astrodyn_bevy/src/components/gravity.rs`, `crates/astrodyn_bevy/src/components/interaction.rs`, `crates/astrodyn_bevy/src/components/mass_tree.rs` plus their re-exporting `crates/astrodyn_bevy/src/components/mod.rs` carry the Bevy `Component` boundary tags; `crates/astrodyn_bevy/src/lib.rs` registers the canonical `AttachEvent<SelfRef, SelfRef>` Bevy message; `crates/astrodyn_runner/src/simulation/types.rs` carries the runner storage tags; `src/derived.rs`, `src/atmosphere.rs`, and `src/planet_config.rs` carry the dynamic-registry-erased return-type tags; `tests/self_ref_self_planet_discipline.rs` is the bidirectional CI lint that refuses any new `SelfRef` / `SelfPlanet` token outside the allow-list mechanism described above) |
