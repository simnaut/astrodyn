# `test_data/jeod_inputs/` — verbatim JEOD source mirror
...
is committed at
`test_data/jeod_inputs/verif/SIM_dyncomp/Modified_data/mass.py`. The
mirror is identical to JEOD's tree, so `diff -r ../jeod/<rel>
test_data/jeod_inputs/<rel>` confirms an unmodified copy.

Resolve a path from Rust via `jeod_test_data::jeod_inputs::path("<rel>")`.

## Refresh after a JEOD upgrade

```bash
# From the bevy_jeod workspace root, with $JEOD_HOME pointing at a
...
mkdir -p "test_data/jeod_inputs/$(dirname "$REL")"
cp "$JEOD/$REL" "test_data/jeod_inputs/$REL"
```

Reference it from a Tier 3 rig with
`jeod_test_data::jeod_inputs::path("path/relative/to/jeod/root")`.
